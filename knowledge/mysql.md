...
- [面试](#面试)
    ...
    - [MySQL性能优化](#MySQL性能优化)
    - [explain和explain analyze的区别](#explain和explainanalyze的区别)
    - [怎么计算的sql语句执行的预计时间](#怎么计算的sql语句执行的预计时间)
    - [MySQL批量插入为什么比逐个插入要快](#MySQL批量插入为什么比逐个插入要快)
...
如果在EXPLAIN中看到有索引合并（Using union），应该检查一下查询和表的结构，说明索引建的很糟糕。
...
### 登录
```SQL
mysql -h (sever——ip) -P (port) -u(username) -p(password)
```
...
## MySQL性能优化
[MySQL性能优化](#如何优化)
## explain和explainanalyze的区别
explain执行查询计划，只能预估查询执行时间；explain analyze会实际执行查询，可以获得实际查询执行时间。
## 怎么计算sql语句执行的预计时间
以偏概全：其预估扫描的数据页分别是前后两个数据页，以及从左边开始连续8个数据页，得到平均每个page的行数，根据总的page个数预估出这个范围的数据行数。
## MySQL批量插入为什么比逐个插入要快
